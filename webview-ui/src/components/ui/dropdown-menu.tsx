...
>(({ className, sideOffset = 4, container, ...props }, ref) => (
	<DropdownMenuPrimitive.Portal container={container}>
		<DropdownMenuPrimitive.Content
			...
			className={cn(
				"z-50 min-w-[8rem] overflow-hidden rounded-md p-1 shadow-xs",
...
>(({ className, inset, ...props }, ref) => (
	<DropdownMenuPrimitive.Item
		...
		className={cn(
			...
			"rounded-md active:opacity-90 cursor-pointer",
			inset && "pl-8",
			className,
		)}
		{...props}
...
>(({ className, children, ...props }, ref) => (
	<DropdownMenuPrimitive.RadioItem
		ref={ref}
		className={cn(
			"relative flex cursor-default select-none items-center rounded-md py-1.5 pl-8 pr-2 text-sm outline-none transition-colors focus:bg-vscode-list-activeSelectionBackground focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
			className,
		)}
		{...props}>
		<span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
