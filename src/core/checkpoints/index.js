...
export function getCheckpointService(cline) {
...
async function getInitializedCheckpointService(cline, { interval = 250, timeout = 15_000 } = {}) {
...
export async function checkpointSave(cline) {
	const service = getCheckpointService(cline)
	if (!service) {
		return
	}
...
export async function checkpointRestore(cline, { ts, commitHash, mode }) {
...
export async function checkpointDiff(cline, { ts, previousCommitHash, commitHash, mode }) {
	const service = await getInitializedCheckpointService(cline)
	if (!service) {
		return
	}
