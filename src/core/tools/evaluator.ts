// Tipos Roo-first locais (mínimos necessários)
export type AnswerAction = { answer: string }
...
export type EvaluationResponse = { pass: boolean; think?: string; reason?: string; [key: string]: any }
// Função utilitária local para extrair conhecimento em string
...
// Função Roo-first: apenas gera o prompt para avaliação, sem executar modelo herdado
function buildEvaluationPrompt(
	...
	switch (evaluationType) {
		case "definitive":
			return getDefinitivePrompt(question, answer.answer)
		case "freshness":
			return getFreshnessPrompt(question, answer, new Date().toISOString())
		case "plurality":
			return getPluralityPrompt(question, answer.answer)
		case "completeness":
			return getCompletenessPrompt(question, answer.answer)
		case "strict":
			return getRejectAllAnswersPrompt(question, answer, allKnowledge)
		default:
...
// Função Roo-first: retorna prompts para todos os tipos de avaliação solicitados
