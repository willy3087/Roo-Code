...
export async function executeCommand(
	cline,
	{ executionId, command, customCwd, terminalShellIntegrationDisabled = false, terminalOutputLineLimit = 500 },
) {
	let workingDir
	if (!customCwd) {
	...
	const clineProvider = await cline.providerRef.deref()
	const callbacks = {
		onLine: async (output, process) => {
			const status = { executionId, status: "output", output }
			clineProvider?.postMessageToWebview({ type: "commandExecutionStatus", text: JSON.stringify(status) })
			if (runInBackground) {
				return
			}
