...
describe("mentions", () => {
	...
	describe("parseMentions", () => {
		let mockUrlFetcher
		beforeEach(() => {
			mockUrlFetcher = new UrlContentFetcher()
			fs.stat.mockResolvedValue({ isFile: () => true, isDirectory: () => false })
			require("../../../integrations/misc/extract-text").extractTextFromFile.mockResolvedValue(
				"Mock file content",
			)
		})
...
- Added tests`
	...
	describe("openMention", () => {
		beforeEach(() => {
			getWorkspacePath.mockReturnValue(mockCwd)
		})
		it("should handle URLs", async () => {
			...
			const calledArg = vscode.env.openExternal.mock.calls[0][0]
		...
		it("should unescape folder path before revealing", async () => {
			...
			vscode.Uri.file.mockReturnValue(expectedUri)
			await openMention(mention)
