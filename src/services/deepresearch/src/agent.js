...
export function BuildMsgsFromKnowledge(knowledge) {
...
export function composeMsgs(messages, knowledge, question, finalAnswerPIP) {
...
`)
	...
	if (allowRead && urlList.length > 0) {
		const urlListStr = urlList
			.map(
				(item, idx) =>
					`  - [idx=${idx + 1}] [weight=${item.score.toFixed(2)}] "${item.url}": "${item.merged.slice(0, 50)}"`,
			)
			.join("\n")
		actionSections.push(`
<action-visit>
...
const allContext = [] // all steps in the current session, including those leads to wrong results
export function updateContext(step) {
	allContext.push(step)
}
export async function updateReferences(thisStep, allURLs) {
	thisStep.references = thisStep.references
		...
		.filter(Boolean) // Add type assertion here
...
export async function executeSearchQueries(keywordsQueries, context, allURLs, SchemaGen, onlyHostnames) {
	...
	for (const query of keywordsQueries) {
		...
		const minResults = results
			...
			.filter(Boolean) // Filter out null entries and assert type
	...
	if (searchedQueries.length === 0) {
		if (onlyHostnames && onlyHostnames.length > 0) {
			console.log(
				`No results found for queries: ${uniqQOnly.join(", ")} on hostnames: ${onlyHostnames.join(", ")}`,
			)
			context.actionTracker.trackThink("hostnames_no_results", SchemaGen.languageCode, {
...
export function includesEval(allChecks, evalType) {
...
export async function getResponse(
	...
	let schema = SchemaGen.getAgentSchema(true, true, true, true, true)
	const gaps = [question] // All questions to be answered including the orginal question
	...
	while (context.tokenTracker.getTotalUsage().totalTokens < regularBudget) {
		...
		if (currentQuestion.trim() === question && totalStep === 1) {
			...
			evaluationMetrics[currentQuestion] = (await evaluateQuestion(currentQuestion, context, SchemaGen)).map(
				(e) => {
					return {
						type: e,
		...
		try {
			...
			if (error && typeof error === "object") {
				error.originalQuery = currentQuestion
			}
			throw error
		}
		// print allowed and chose action
		const actionsStr = [allowSearch, allowRead, allowAnswer, allowReflect, allowCoding]
		...
		if (thisStep.action === "answer" && thisStep.answer) {
			...
			if (thisStep.references.length > 0) {
				const urls =
					thisStep.references?.filter((ref) => !visitedURLs.includes(ref.url)).map((ref) => ref.url) || []
			...
			let evaluation = { pass: true, think: "" }
...
`)
			...
			const uniqQOnly = chooseK(
				(await dedupQueries(qOnly, allKeywords, context.tokenTracker)).unique_queries,
				MAX_QUERIES_PER_STEP,
			)
...
`)
			...
			// normalize URLs
			thisStep.URLTargets = thisStep.URLTargets.map((idx) => normalizeUrl(urlList[idx - 1])).filter(
				(url) => url && !visitedURLs.includes(url),
			)
			thisStep.URLTargets = [...new Set([...thisStep.URLTargets, ...weightedURLs.map((r) => r.url)])].slice(
				0,
				MAX_URLS_PER_STEP,
			)
...
`)
	...
	if (!trivialQuestion) {
		thisStep.mdAnswer = repairMarkdownFinal(
			convertHtmlTablesToMd(
				fixBadURLMdLinks(
					fixCodeBlockIndentation(
						repairMarkdownFootnotesOuter(
							await repairUnknownChars(
								await fixMarkdown(buildMdFromAnswer(thisStep), allKnowledge, context, SchemaGen),
								context,
							),
						),
					),
		...
		thisStep.mdAnswer = convertHtmlTablesToMd(fixCodeBlockIndentation(buildMdFromAnswer(thisStep)))
	}
	console.log(thisStep)
	// max return 300 urls
...
export async function storeContext(prompt, schema, memory, step) {
	const { allContext, allKeywords, allQuestions, allKnowledge, weightedURLs, msgWithKnowledge } = memory
	if (process.asyncLocalContext?.available?.()) {
		process.asyncLocalContext.ctx.promptContext = {
			prompt,
			schema,
			allContext,
			allKeywords,
...
export async function main() {
	...
	const { result: finalStep, context: tracker, visitedURLs: visitedURLs } = await getResponse(question)
