...
function calculateStats(results, modelName) {
...
async function evaluateAnswer(expectedAnswer, actualAnswer) {
...
async function batchEvaluate(inputFile) {
	// Read and parse input file
	const questions = JSON.parse(await fs.readFile(inputFile, "utf-8"))
	const results = []
	...
	for (let i = 0; i < questions.length; i++) {
		...
		try {
			...
			const { result: response, context } = await getResponse(question)
...
//# sourceMappingURL=batch-evals.js.map
