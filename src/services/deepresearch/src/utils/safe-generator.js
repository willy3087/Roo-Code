...
export class ObjectGeneratorSafe {
	...
	createDistilledSchema(schema) {
		...
		if (typeof schema === "object" && schema !== null) {
			return this.stripSchemaDescriptions(schema)
		}
	 ...
	 */
	stripZodDescriptions(zodSchema) {
		...
		if (zodSchema instanceof z.ZodArray) {
			return z.array(this.stripZodDescriptions(zodSchema._def.type))
		}
	...
	async generateObject(options) {
		...
		try {
			...
			try {
				const errorResult = await this.handleGenerateObjectError(error)
				...
				if (numRetries > 0) {
					...
					try {
						...
						try {
							const lastChanceResult = await this.handleGenerateObjectError(fallbackError)
			...
			if (!textToParse || typeof textToParse !== "string") {
				...
				try {
					...
					const emptyAgentResponse = {
						action: "search",
						think: "Error recovery mode: performing search for additional information",
						searchRequests: [
							// Reutilizar a última pergunta como termo de busca
							error.originalQuery || "informações adicionais necessárias",
						],
					}
					console.log("Fallback to empty agent response:", emptyAgentResponse)
					return {
						object: emptyAgentResponse,
						usage: error.usage || {
							prompt_tokens: 0,
							completion_tokens: 0,
							total_tokens: 0,
			...
			try {
				...
				try {
					...
					try {
						...
						const actionMatch = textToParse.match(/["']?action["']?\s*:\s*["']?(\w+)["']?/i)
						const thinkMatch = textToParse.match(/["']?think["']?\s*:\s*["']?([^"']+)["']?/i)
						if (actionMatch && actionMatch[1]) {
							const action = actionMatch[1].toLowerCase()
							const think = thinkMatch ? thinkMatch[1] : "Emergency recovery mode"
							// Construir objeto de resposta mínimo com base na ação detectada
							const recoveryObject = { action, think }
							// Adicionar propriedades baseadas na ação detectada
							if (action === "search") {
								recoveryObject.searchRequests = [error.originalQuery || "informações adicionais"]
							} else if (action === "answer") {
								recoveryObject.answer = "Não foi possível gerar uma resposta estruturada."
...
//# sourceMappingURL=safe-generator.js.map
