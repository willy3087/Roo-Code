...
export function buildMdFromAnswer(answer) {
	return repairMarkdownFootnotes(answer.answer, answer.references)
}
export function repairMarkdownFootnotes(markdownString, references) {
	...
	if (validFootnotes.length === 0) {
		const appendedCitations = Array.from({ length: references.length }, (_, i) => `[^${i + 1}]`).join("")
...
`.trim()
	...
	let currentIndex = 0
	const correctedMarkdown = cleanedMarkdown.replace(standardFootnoteRegex, () => `[^${++currentIndex}]`)
	return `
${correctedMarkdown}
 ...
 */
export function repairMarkdownFootnotesOuter(markdownString) {
...
export const removeExtraLineBreaks = (text) => {
	return text.replace(/\n{2,}/gm, "\n\n")
}
export function chooseK(a, k) {
...
export function getI18nText(key, lang = "pt-BR", params = {}) {
	// 获取i18n数据
...
export function smartMergeStrings(str1, str2) {
...
export function fixCodeBlockIndentation(markdownText) {
 ...
 */
export function convertHtmlTablesToMd(mdString) {
	try {
		let result = mdString
 ...
 */
function convertSingleHtmlTableToMd(htmlTable) {
	try {
		...
		// Extract headers
		const headers = Array.from(table.querySelectorAll("thead th")).map((th) => sanitizeCell(th.textContent || ""))
				...
				if (containsMarkdown) {
					...
					if (cellContent.includes("* ") || cellContent.includes("*  ") || cellContent.includes("*   ")) {
						...
						if (td.querySelectorAll("li").length > 0) {
							// Handle HTML lists
							listItems = Array.from(td.querySelectorAll("li")).map((li) => li.innerHTML.trim())
						} else {
 ...
 */
function processFormattedText(text, openMarker, closeMarker) {
 ...
 */
export function repairMarkdownFinal(markdown) {
	try {
