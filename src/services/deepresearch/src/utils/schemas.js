...
function getLanguagePrompt(question) {
	return {
...
export class Schemas {
	languageStyle = "informal Portuguese"
	languageCode = "pt-BR"
	async setLanguage(query) {
	...
	getQuestionEvaluateSchema() {
		return z.object({
			think: z
				.string()
				.describe(`A very concise explain of why those checks are needed. ${this.getLanguagePrompt()}`)
				.max(500),
	...
	getErrorAnalysisSchema() {
		return z.object({
			recap: z
				.string()
				.describe("Recap of the actions taken and the steps conducted in first person narrative.")
				.max(500),
			blame: z
				.string()
				.describe(
	...
	getQueryRewriterSchema() {
		return z.object({
			think: z
				.string()
				.describe(`Explain why you choose those search queries. ${this.getLanguagePrompt()}`)
				.max(500),
			queries: z
				.array(
					z.object({
						...
						q: z
							.string()
							.describe("keyword-based search query, 2-3 words preferred, total length < 30 characters")
							.max(50),
					}),
				)
				...
				return z.object({
					...
					freshness_analysis: z.object({
						days_ago: z
							.number()
							.describe(
								`datetime of the **answer** and relative to ${new Date().toISOString().slice(0, 10)}.`,
							)
							.min(0),
						max_age_days: z
							.number()
				...
				return z.object({
					...
					plurality_analysis: z.object({
						minimum_count_required: z
							.number()
							.describe("Minimum required number of items from the **question**"),
						...
						.array(
							z
								.object({
									...
									dateTime: z
										.string()
										.describe("Use original message's <answer-dateime> if available.")
										.max(16),
								})
								.required(),
						)
...
Ensure each reflection question:
 ...
 - NEVER pose general questions like: "How can I verify the accuracy of information before including it in my answer?", "What information was actually contained in the URLs I found?", "How can i tell if a source is reliable?".
		...
		return z.object({
			think: z
				.string()
				.describe(`Concisely explain your reasoning process in ${this.getLanguagePrompt()}.`)
				.max(500),
			action: z
				.enum(Object.keys(actionSchemas).map((key) => key))
				.describe(
