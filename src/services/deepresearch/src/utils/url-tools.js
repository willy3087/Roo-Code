...
export function filterURLs(allURLs, visitedURLs, badHostnames, onlyHostnames) {
	return Object.entries(allURLs)
		.filter(
			([url]) =>
				!visitedURLs.includes(url) &&
				!badHostnames.includes(extractUrlParts(url).hostname) &&
				(onlyHostnames.length === 0 || onlyHostnames.includes(extractUrlParts(url).hostname)),
		)
		.map(([, result]) => result)
}
// Function to extract hostname and path from a URL
const extractUrlParts = (urlStr) => {
	try {
...
export const countUrlParts = (urlItems) => {
	const hostnameCount = {}
	const pathPrefixCount = {}
	let totalUrls = 0
...
const normalizeCount = (count, total) => {
	return total > 0 ? count / total : 0
}
// Calculate boosted weights
export const rankURLs = (urlItems, options = {}, trackers) => {
	...
	if (question.trim().length > 0) {
		...
		rerankDocuments(question, uniqueContents, trackers.tokenTracker).then(({ results }) => {
			...
			results.forEach(({ index, relevance_score }) => {
				...
				originalIndices.forEach((originalIndex) => {
					urlItems[originalIndex].jinaRerankBoost = boost
				})
			})
		})
	}
...
export const addToAllURLs = (r, allURLs, weightDelta = 1) => {
...
export const sortSelectURLs = (allURLs, maxURLs = 70) => {
 ...
 */
export function sampleMultinomial(items) {
	// Handle empty array
 ...
 */
export async function getLastModified(url) {
	try {
...
export const keepKPerHostname = (results, k) => {
	const hostnameMap = {}
...
export async function processURLs(urls, context, allKnowledge, allURLs, visitedURLs, badURLs, schemaGen, question) {
...
export function fixBadURLMdLinks(mdContent, allURLs) {
...
export function extractUrlsWithDescription(text, contextWindowSize = 50) {
