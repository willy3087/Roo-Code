...
app.use(
	cors({
		origin: [
			...
			"http://localhost:4001", // http-MCP
			"http://localhost:4000", // SSE-MCP
...
export async function* streamTextNaturally(text, streamingState) {
...
export function calculateDelay(chunk, burstMode) {
...
export function getEffectiveLength(chunk) {
...
export async function emitRemainingContent(res, requestId, created, model, content) {
	if (!content) return
	const chunk = {
		id: requestId,
...
export function getTokenBudgetAndMaxAttempts(reasoningEffort = "medium", maxCompletionTokens = null) {
...
export async function completeCurrentStreaming(streamingState, res, requestId, created, model) {
	if (streamingState.currentlyStreaming && streamingState.remainingContent) {
		// Force completion of current streaming
		await emitRemainingContent(res, requestId, created, model, streamingState.remainingContent)
...
export async function processQueue(streamingState, res, requestId, created, model) {
...
app.post("/v1/chat/completions", async (req, res) => {
	...
	body.messages = body.messages?.filter((message) => {
		if (message.role === "assistant") {
			...
			if (typeof message.content === "string") {
				...
				message.content.forEach((content) => {
					if (content.type === "text") {
						content.text = content.text.replace(/<think>[\s\S]*?<\/think>/g, "").trim()
					}
				})
				//Filter out any content objects in the array that now have null/undefined/empty text.
				message.content = message.content.filter((content) => !(content.type === "text" && content.text === ""))
			...
			if (Array.isArray(message.content)) {
				message.content = message.content.map((content) => `${content.text || content}`).join(" ")
			}
			return true
		}
	...
	if (body.response_format?.json_schema) {
		...
		try {
			...
			res.status(400).json({ error: `Invalid JSON schema: ${error.message}` })
			return
		}
	}
	...
	if (body.stream) {
		...
		const actionListener = async (step) => {
	...
	try {
		...
		let finalAnswer = finalStep.mdAnswer
		const annotations = finalStep.references?.map((ref) => ({
			type: "url_citation",
			url_citation: {
				title: ref.title,
