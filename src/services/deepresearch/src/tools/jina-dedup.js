...
async function getEmbeddings(queries) {
	...
	try {
		...
		const embeddings = response.data.data.sort((a, b) => a.index - b.index).map((item) => item.embedding)
		return {
			embeddings,
...
export async function dedupQueries(newQueries, existingQueries, tracker) {
	try {
...
//# sourceMappingURL=jina-dedup.js.map
