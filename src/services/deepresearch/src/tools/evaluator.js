...
function getRejectAllAnswersPrompt(question, answer, allKnowledge) {
...
function getPluralityPrompt(question, answer) {
	return {
...
function getQuestionEvaluationPrompt(question) {
	return {
...
export async function evaluateQuestion(question, trackers, schemaGen) {
	try {
...
async function performEvaluation(evaluationType, prompt, trackers, schemaGen) {
	const generator = new ObjectGeneratorSafe(trackers.tokenTracker)
	const result = await generator.generateObject({
		model: TOOL_NAME,
...
export async function evaluateAnswer(question, action, evaluationTypes, trackers, allKnowledge, schemaGen) {
	let result
	for (const evaluationType of evaluationTypes) {
		...
		if (prompt) {
			result = await performEvaluation(evaluationType, prompt, trackers, schemaGen)
