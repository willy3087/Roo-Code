 ...
 */
export async function repairUnknownChars(mdContent, trackers) {
	...
	while (remainingUnknowns && iterations < 20) {
		...
		if (position === lastPosition) {
			...
			repairedContent = repairedContent.substring(0, position) + repairedContent.substring(position + 1)
			continue
		}
...
So what was the original text between these two contexts?`,
			...
			if (replacement === "UNKNOWN" || replacement.includes("�") || replacement.length > unknownCount * 4) {
...
//# sourceMappingURL=broken-ch-fixer.js.map
